...
class Hand {
  ...
  _checkType() {
    ...
    if (RegExp(r'(.)\1{3}').hasMatch(sortedHand)) {
      if (sortedHand.contains('J')) return Type.FiveKind;
      return Type.FourKind;
    }
    var foo = RegExp(r'(.)\1{2}').firstMatch(sortedHand);
    if (foo != null &&
        RegExp(r'([^' + foo.group(0)! + r'])\1{1}').hasMatch(sortedHand)) {
      int jokers = RegExp('[J]').allMatches(sortedHand).length;
      if (jokers == 3 || jokers == 2) return Type.FiveKind;
      return Type.Full;
    }
    if (RegExp(r'(.)\1{2}').hasMatch(sortedHand)) {
      if (sortedHand.contains('J')) return Type.FourKind;
      return Type.ThreeKind;
    }
    if (RegExp(r'(.)\1{1}').allMatches(sortedHand).length == 2) {
      int jokers = RegExp('[J]').allMatches(sortedHand).length;
      if (jokers == 2) return Type.FourKind;
      if (jokers == 1) return Type.Full;
      return Type.TwoPair;
    }
    if (RegExp(r'(.)\1{1}').allMatches(sortedHand).length == 1) {
      if (sortedHand.contains('J')) return Type.ThreeKind;
      return Type.OnePair;
    }
    if (sortedHand.contains('J')) return Type.OnePair;
