...
void main() {
  ...
  _examineLoop(Tile start) {
    ...
    Tile prev = start;
    Set<Tile> path = {trueStart, start, prev};
    int cost = 0;
  ...
  int _examineLine(List<Tile> line) {
    if (line.toSet().intersection(finalPath).length >= 2) {
      int inside = 0;
      bool isInside = false;
      bool isInLine = false;
      Tile? startingTile;
      ...
      for (var tile in line) {
        if (finalPath.contains(tile)) {
          if (startingTile == null) {
            ...
            if (tile.E == null) {
              isInside = startingTile.keepPrevVal(tile) ? isInside : !isInside;
              startingTile = null;
              isInLine = false;
            }
          }
        } else if (isInside && !isInLine) {
          inside++;
        }
      }
      return inside;
  ...
  int allInside = 0;
  for (var line in tilesMap) {
    allInside += _examineLine(line);
  }
  print(allInside);
}
